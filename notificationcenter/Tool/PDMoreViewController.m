...
- (void)viewDidLoad
{
    ...
    for (ComboxDatas *cDatas in self.datasource.result) {
        if (TypeSource == [cDatas.type integerValue]) {
        ...
        if (TypeHouseType == [cDatas.type integerValue]) {
            for (int i=0; i<cDatas.datas.count; i++) {
                ComboxData *data = (ComboxData *)cDatas.datas[i];
                [self.segHouseType insertSegmentWithTitle:data.name atIndex:i animated:NO];
            }
        }
        if (TypeOrder == [cDatas.type integerValue]) {
            for (int i=0; i<cDatas.datas.count; i++) {
                ComboxData *data = (ComboxData *)cDatas.datas[i];
...
{
    NSNumber *sourceId=[self getSegSelectedId:TypeSource text:[self.segSource titleForSegmentAtIndex:self.segSource.selectedSegmentIndex]];
