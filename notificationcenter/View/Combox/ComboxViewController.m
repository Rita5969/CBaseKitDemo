...
#define FRComboxViewPointY 64 //combox的Y轴位置
#define FRComboxViewHeight 50 //combox的高度
#define FRBetweenPD 7 //combox与下拉列表的间隔
#define PDBoundsDistance 10 //下拉列表与两侧的距离之和
#define PDHeightAnimateBefore 0 //下拉列表的高度
#define PDHeightAnimateAfter screen.size.height - FRComboxViewHeight - 64-20 //下拉列表的高度
...
#define PDTBLFrameBefore CGRectMake(0,0,PDFrameBefore.size.width,PDFrameBefore.size.height)
...
{
    FRComboxViewController *_frController;
...
- (void)viewDidLoad
{
    ...
    _comboxVo = [WebRequest getComboxVo:@"http://115.29.6.60/download/data.txt"];
    ...
    [_filterParams setObject:@1 forKey:@"distanceId"];
    ...
    CGRect frViewFrame=CGRectMake(0, 0, screen.size.width, FRComboxViewHeight);
    
    _frController=[[FRComboxViewController alloc]init];
    _frController.view.backgroundColor=[UIColor orangeColor];
    _frController.view.frame=ctrlFrame;
    ...
    [self.view addSubview:_frController.view];
    ...
    //添加筛选下拉列表
    _pdMoreViewController=[[PDMoreViewController alloc]initWithDataSource:_comboxVo];
